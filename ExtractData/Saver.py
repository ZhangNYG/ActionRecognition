...
class Tracker:
    def __init__(self,len_lim=10,dist_thresh=40,frame_th = 5):
        self.tracks = []
    ...
    def saveAction(self,root):
        for track in self.tracks:
            if track.getLen() >= 10:
                ...
                file = open(root+"/data.txt",mode='a+')
                np.savetxt(file,data_sample,fmt="%.7f",delimiter=',')
                track.delFirst()
